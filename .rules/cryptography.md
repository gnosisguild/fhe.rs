...
2.6. BFV and l-BFV encryption error sampling is asymmetric: `e1` comes from `Poly::conditional_error(error1_variance)` (CBD or uniform branch, via `Poly::error_1` in `lbfv::LBFVPublicKey::try_encrypt`/`try_encrypt_extended`) while `u` and `e2` use `Poly::small(params.variance)` (CBD). l-BFV public-key *generation* (`from_crs`, `new_with_seed`) still samples its noise from `params.variance`, matching `bfv::PublicKey::new`, which also does not use `error1_variance`. Any bound depending on `B_enc` must account for the branch actually used, and `e1`'s bound can differ from `u`/`e2`'s when `error1_variance != variance` (issue #56).
...
4.4. `B_enc` must be derived from the actual `Poly::conditional_error` branch: CBD iff `v <= 16` with support bound `2 * v`, otherwise uniform with `variance_to_uniform_bound` (the smallest `B` with `B(B+1)/3 >= v`). `fhe_math::rq::error_support_bound` is the single canonical source for this bound; the sampler and trBFV's `compute_b_enc` must consume it (or its shared private spec) rather than re-deriving branch conditions locally, and zero variance is rejected explicitly.
...
4.9. Threshold decryption follows the level-zero contract: `decryption_share` rejects 3-component (unrelinearized) ciphertexts, clears variable-time flags on ciphertext `c0` and `c1` before multiplying `c1` by secret material and adding the result, and `SecretPoly` normalizes flags on construction so every operand in the secret-bearing chain remains constant-time. `decrypt_from_shares` validates the plaintext modulus through the representation-independent `try_plaintext` check (typed `PlaintextModulusNotU64` above `u64::MAX`) and requires it to construct the u64 `Modulus` used for the final reduction (`Modulus::new` rejects values at/above `2^62`, reported as a typed error); the coefficient accumulation uses checked addition so out-of-contract values can never overflow.
...
7.8. trBFV `compute_b_enc` in `crates/fhe/src/trbfv/smudging.rs` derives `B_enc` only as a thin fallible adapter over `fhe_math::rq::error_support_bound` (CBD support `2v` for `v <= 16`, exact minimal uniform bound above; issue #167). Reviewers should reject any local re-derivation of the branch boundary, float square roots, unchecked conversions, or infallible signatures in this path.
