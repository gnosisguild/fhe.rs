...
2.7. `Poly::small` requires variance in `1..=16`; an `error1_variance` above 16 selects the uniform branch of `conditional_error`. `BfvParametersBuilder` enforces the CBD domain (`set_variance` and `build` accept exactly `1..=16`), rejects a zero `error1_variance` through every setter and at `build` (no fixed upper bound), and validates supplied ciphertext moduli (nonempty, valid, distinct, pairwise coprime, NTT-friendly) plus a nonzero plaintext modulus before any context allocation; the polynomial degree must be a power of two in `[8, 65536]`.
