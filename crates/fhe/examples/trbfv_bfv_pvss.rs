...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!("🔢 Collecting encrypted shares (homomorphic addition) and decrypting...");

    timeit!(
        "Share collection (sum encrypted) and decryption (parallel)",
        {
            parties
                .par_iter_mut()
                .enumerate()
                .for_each(|(receiver_idx, party)| {
                    // For each modulus level, sum all encrypted shares from all senders
                    let num_moduli = params_trbfv.moduli().len();
                    let trb_moduli = params_trbfv.moduli();

                    // 1. Sum encrypted shares in BFV space
                    // Sum encrypted sk shares - one ciphertext per modulus level
                    ...
                    for m in 0..num_moduli {
                        ...
                        for sender_encrypted in encrypted_shares.iter().take(num_parties) {
                            let (encrypted_sk_shares, _) = &sender_encrypted[receiver_idx];
                            sum_ct += &encrypted_sk_shares[m];
                        }
                        summed_sk_cts.push(sum_ct);
                    }

                    // Sum encrypted esi shares - one ciphertext per modulus level
                    let mut summed_esi_cts: Vec<Ciphertext> = Vec::new();
                    for m in 0..num_moduli {
                        let mut sum_ct = Ciphertext::zero(&params_bfv);
                        for sender_encrypted in encrypted_shares.iter().take(num_parties) {
                            let (_, encrypted_esi_shares) = &sender_encrypted[receiver_idx];
                            sum_ct += &encrypted_esi_shares[m];
                        }
                        summed_esi_cts.push(sum_ct);
                    }

                    // Now decrypt the summed ciphertexts once per modulus level
                    // and perform modular reduction with respect to trBFV moduli
                    // Result is: (share_1 + share_2 + ... + share_N) mod BFV_plaintext_modulus
                    let mut node_share_m = Array::zeros((0, degree));
                    for (m, ct) in summed_sk_cts.iter().enumerate() {
                        let pt = party.sk_bfv.try_decrypt(ct).unwrap();
                        let mut decrypted_share: Vec<u64> =
                            Vec::<u64>::try_decode(&pt, Encoding::poly()).unwrap();

                        // 3. Reduce to trBFV modulus space for Shamir reconstruction
                        let modulus = trb_moduli[m];
                        for val in decrypted_share.iter_mut() {
                            *val %= modulus;
                        }
                    ...
                    // Now it's: (share_1 + share_2 + ... + share_N) mod trBFV_modulus

                    // Store as a single "aggregated" share from all parties
                    party.sk_sss_collected.push(node_share_m);

                    let mut es_node_share_m = Array::zeros((0, degree));
                    for (m, ct) in summed_esi_cts.iter().enumerate() {
                        let pt = party.sk_bfv.try_decrypt(ct).unwrap();
                        let mut decrypted_share: Vec<u64> =
                            Vec::<u64>::try_decode(&pt, Encoding::poly()).unwrap();

                        // CRITICAL: Reduce modulo the corresponding trBFV modulus
                        let modulus = trb_moduli[m];
                        for val in decrypted_share.iter_mut() {
                            *val %= modulus;
                        }
