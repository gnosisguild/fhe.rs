...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mut params_bfv_sets: Vec<Arc<bfv::BfvParameters>> = Vec::new();

    for trbfv_modulo in moduli_trbfv {
        params_bfv_sets.push(bfv::BfvParametersBuilder::new()
                .set_degree(degree)
                .set_plaintext_modulus(trbfv_modulo)
    ...
    for params_bfv in params_bfv_sets.iter() {
        println!("  BFV ciphertext moduli: {:?}", params_bfv.moduli());
        println!("  BFV plaintext moduli: {:?}", params_bfv.plaintext());
    }
    ...
    struct Party {
        ...
        sk_bfv: Vec<SecretKey>,
        pk_bfv: Vec<PublicKey>,
    }
    ...
    let mut parties: Vec<Party> = timeit!("Party setup (parallel)", {
        (0..num_parties)
            ...
            .map(|_| {
                ...
                // SecretKey::random uses just the degree so we can pass whatever bfv set.
                let mut sk_bfv = Vec::new();
                for params_bfv in params_bfv_sets.iter() {
                    sk_bfv.push(SecretKey::random(params_bfv, &mut rng));
                }
                let mut pk_bfv = Vec::new();
                for sk_bfv in sk_bfv.iter() {
                    pk_bfv.push(PublicKey::new(sk_bfv, &mut thread_rng));
                }
    ...
    let pk_bfv_list: Vec<Vec<PublicKey>> = parties.iter().map(|p| p.pk_bfv.clone()).collect();
                ...
                .map(|(_sender_idx, party)| {
                        ...
                        pk_bfv_list.iter().enumerate().take(num_parties)
                    {
                        ...
                        for m in 0..params_trbfv.moduli().len() {
                            ...
                            let pt =
                                Plaintext::try_encode(&share_vec, Encoding::poly(), &params_bfv_sets[m])
                                    .unwrap();
                            let ct = receiver_pk[m].try_encrypt(&pt, &mut rng).unwrap();
            ...
            .for_each(|(receiver_idx, party)| {
                for sender_encrypted in encrypted_shares.iter().take(num_parties) {
                    ...
                    for (m, ct) in encrypted_sk_shares.iter().enumerate() {
                        let pt = party.sk_bfv[m].try_decrypt(ct).unwrap();
