...
pub enum ThresholdError {
    /// The number of parties is below the minimum supported by the protocol.
    #[error("invalid party count: got {actual}, minimum is {minimum}")]
    InvalidPartyCount { actual: usize, minimum: usize },
...
pub enum EvaluationKeyError {
    ...
    #[error("Public key contains no ciphertexts")]
    EmptyPublicKey,

    #[error("Unsupported public-key polynomial representation {found}")]
    UnsupportedRepresentation { found: String },

    #[error("Invalid decomposition length: expected {expected}, found {actual}")]
    InvalidDecompositionLength { actual: usize, expected: usize },
...
pub enum SerializationError {
    ...
    /// A serialized public-key seed has the wrong length.
...
pub enum ParametersError {
    ...
    /// Indicates that the threshold-BFV error variance could not be parsed.
    ...
    /// Indicates that an operation requires a machine-word plaintext modulus.
    #[error("Unsupported plaintext modulus: {reason}")]
    UnsupportedPlaintextModulus { reason: String },
