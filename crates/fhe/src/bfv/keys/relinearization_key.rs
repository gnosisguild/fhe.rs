...
impl RelinearizationKey {
    ...
    /// Relinearize an extended ciphertext `(c₀, c₁, c₂)` in place.
    ///
    /// After homomorphic multiplication, a degree-2 ciphertext satisfies
    /// `c₀ + c₁·s + c₂·s² ≈ m + e`. Relinearization produces `(c₀', c₁')` with
    /// `c₀' + c₁'·s ≈ m + e'` by key-switching `c₂` under `s² → s` and adding
    /// the result to `(c₀, c₁)`.
    pub fn relinearizes(&self, ct: &mut Ciphertext) -> Result<()> {
        ct.validate_for(&self.ksk.params)?;
        if ct.len() != 3 {
