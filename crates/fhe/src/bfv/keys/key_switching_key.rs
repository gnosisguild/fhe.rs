//! Key-switching keys for the BFV encryption scheme. Adapts the Brakerski-Vaikuntanathan key
//! switching through decomposition technique adapted to RNS as described in the HPS optimization paper
...
impl KeySwitchingKey {
    ...
    /// Generate the c0 component of the key switching key (KSK) using the Brakerski-Vaikuntanathan key
    /// switching through decomposition technique adapted to RNS as described in the HPS optimization paper
    /// (https://eprint.iacr.org/2018/117).
    ///
    /// A key switching key consists of two components (c0,c1) = (KS_0, KS_1). This function generates
    /// the KS_0 component while KS_1 is generated separately as random polynomials.
    ///
    ...
    /// - g_i is the RNS basis conversion factor (q̃_i · q*_i) for the current modulus q_i
    ///
    /// Each element of KS_0 therefore corresponds to operations performed in RNS,
    /// with its own error polynomial e_i and random polynomial a_i, resulting in a collection of RNS
    /// polynomials that form the complete KS_0 component.
    ///
    /// # Arguments
    ///
    /// * `sk` - The secret key
    ...
    /// # Returns
    ///
    /// * `Result<Vec<Poly>>` - The generated KS_0 polynomials, where each element corresponds to
    ///                         an RNS polynomial
    ///
    /// # Errors
    ///
        ...
        let c0 = c1
            ...
            .map(|(i, c1i)| {
                ...
                *a_s.as_mut() *= s.as_ref();
                ...
                // g_i_from = poly * qi_tilde * qi_star
                // We expect that every RNS component of g_i_from has all zero coefficients except for the i-th RNS component
    ...
    /// Key switch a polynomial from one secret key to another using the Brakerski-Vaikuntanathan key
    /// switching through decomposition technique adapted to RNS as described in the HPS optimization paper
    /// (https://eprint.iacr.org/2018/117).
    ///
    /// This function performs key switching on a polynomial `p` encrypted under one secret key to obtain
    /// a ciphertext encrypted under a different secret key. Unlike key switching a full ciphertext,
    /// this function only handles the switching of a single polynomial (typically C1 of a ciphertext)
    /// and does not add any existing C0 term to the result. This makes it more flexible for use in
    /// various homomorphic operations where different handling of the C0 term may be desired.
    ///
    /// The function supports two modes of operation:
    /// - When `log_base = 0`: Direct key switching without decomposition (hybrid key switching)
    /// - When `log_base > 0`: Key switching with base-2^log_base decomposition (RNS decomposition key switching)
    ///
    /// # Arguments
    ///
    /// * `p` - The input polynomial to key switch, must be in power basis representation
    ///
    /// # Returns
    ///
    /// * `Result<(Poly, Poly)>` - A tuple containing (C0, C1) of the key switched result
    ///
    /// # Errors
    ///
    /// Returns an error if:
    /// * The input polynomial's context doesn't match the expected ciphertext context
    ...
    pub fn key_switch(&self, p: &Poly) -> Result<(Poly, Poly)> {
        ...
        for (c2_i_coefficients, c0_i, c1_i) in izip!(
            ...
            // Takes the coefficients of [p]_{qi} and converts them to an RNS representation by taking [[p]_qi]_qj for every RNS basis qj!
            let mut c2_i = unsafe {
