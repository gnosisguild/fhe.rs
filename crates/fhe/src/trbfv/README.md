...
material** that must never be reused across decryptions. Convert the collected
noise matrices with [`TRBFV::aggregate_noise_shares`] and pass the resulting
non-`Clone` value to [`TRBFV::decryption_share`]; consuming it prevents local
accidental reuse. Replay of copied or serialized transport material remains an
