...
> `SmudgingBoundCalculatorConfig::new_multiplicative` →
> `SmudgingBoundCalculator::new` (optionally
> `.with_accepted_participant_count(...)`) →
> `SmudgingNoiseGenerator::from_bound_calculator` →
> `generate_smudging_error`. Sampled noise remains a non-cloneable
> `GeneratedSmudgingNoise` owner that must be dealt with
...
Smudging noise generated by [`SmudgingNoiseGenerator`] (via
[`SmudgingBoundCalculator`] or a hand-set bound) is **one-time
...
let config = SmudgingBoundCalculatorConfig::new_multiplicative(
    params.clone(), n_parties, num_ciphertexts, mult_depth, lambda,
)?;
let generator =
    SmudgingNoiseGenerator::from_bound_calculator(SmudgingBoundCalculator::new(config))?;
let es_noise = generator.generate_smudging_error(&mut rng)?;
