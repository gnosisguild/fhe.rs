...
> **Breaking change:** smudging dealing, aggregation, and decryption now run
> on dedicated owner types. `ShareManager::deal_smudging_noise` consumes a
> `GeneratedSmudgingNoise` and returns one `SmudgingShare` per recipient;
> `ShareManager::aggregate_smudging_shares` consumes the collected shares
> into an `AggregatedSmudgingShare`; `TRBFV::decryption_share` and
> `ShareManager::decryption_share` consume the aggregate (even on failure),
> so one live aggregate cannot back two decryption shares. The previous
> matrix-returning dealing helper and the `Poly`-accepting decryption APIs
> are removed. Transport uses explicit consuming `export` / `from_bytes`
> boundaries on the same `Rq` encoding as every other secret; copied bytes
> are outside the one-live-owner guarantee, and durable replay prevention
> stays with the integrator (see issue #109).
...
material** that must never be reused across decryptions. The safe-Rust API
enforces this for live values: the noise, share, and aggregate owners are
non-cloneable and each transition (dealing, aggregation, decryption —
including failed attempts) consumes the previous owner, so one live
aggregate cannot back two decryption shares. This guarantee ends at the
transport boundary: exported bytes can be copied and reimported, so
durable per-slot reservation and replay prevention are the integrator's
