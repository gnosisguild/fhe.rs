...
//! The Barrett arithmetic kernels and fixed-schedule inversion are designed
//! with a bounded constant-time goal, but rejection sampling, rayon scheduling
//! when enabled, memory allocation, and code outside this crate are not covered
//! by that claim. This unaudited library is not a security proof.
...
pub use error::Error;
pub use field::{Barrett, BarrettBackend, BarrettField, Field, FieldBackend};
